...
const PatientPage = (): JSX.Element => {
  ...
  const onFormSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    assert(token)
    if (id === 'new') {
      await tryCreatePatient()
    } else {
      await tryUpdatePatient()
    }
  }
  const tryUpdatePatient = async (): Promise<void> => {
    assert(token)
    try {
      const data = await updatePatient(token, patient)
      setPatient(data)
    } catch (error) {
      console.error(error)
  ...
  const tryCreatePatient = async (): Promise<void> => {
    assert(token)
    try {
  ...
  return (
    <div>
      {(id === 'new' && <h1>Création d'un patient</h1>) ||
